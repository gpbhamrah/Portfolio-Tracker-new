    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 px-1">
        ...
        <div className="text-[11px] font-mono text-slate-500 dark:text-slate-400">
          TRACKING: <span className="font-bold text-indigo-600 dark:text-indigo-400">{filteredWatchlist.length}</span> / {watchlist.length} OPPORTUNITIES
        </div>
      </div>
              ...
              return (
                ...
                >
                  ...
                  <td className="p-3.5">
                    ...
                    <div className="text-[11px] font-mono text-slate-500 dark:text-slate-400 uppercase font-semibold mt-0.5">
                      {item.ticker}
                    </div>
                  </td>
                  ...
                  {/* Added Date */}
                  <td className="p-3.5 hidden lg:table-cell text-slate-500 dark:text-slate-400 font-mono">
                  ...
                  {/* Target Entry */}
                  <td className="p-3.5 text-right font-mono text-indigo-600 dark:text-indigo-400 font-bold">
                  ...
                  <td className="p-3.5 text-right">
                    <div className="flex items-center justify-end gap-1 font-mono">
                      {isBelowEntry ? (
                        <span className="inline-flex items-center gap-0.5 text-[11px] font-bold px-2 py-0.5 rounded bg-emerald-50 dark:bg-emerald-950/80 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800 animate-pulse">
                          <TrendingDown className="w-3 h-3" /> BUY ZONE ({Math.abs(diffPercent).toFixed(1)}% below)
                        </span>
                      ) : isNearEntry ? (
                        <span className="inline-flex items-center gap-0.5 text-[11px] font-bold px-2 py-0.5 rounded bg-amber-50 dark:bg-amber-950/80 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-800">
                  ...
                  {/* Notes */}
                  <td className="p-3.5 hidden lg:table-cell text-slate-600 dark:text-slate-400 max-w-xs truncate text-[11px] font-mono">
                    {item.notes || '—'}
                  </td>
